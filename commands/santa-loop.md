...
Run two independent reviewers (flacoAi + an external model) against the current task output. Both must return NICE before the code is pushed. If either returns NAUGHTY, fix all flagged issues, commit, and re-run fresh reviewers — up to 3 rounds.
...
#### Reviewer A: flacoAi Agent (always runs)
...
#### Reviewer B: External Model (flacoAi fallback only if no external CLI installed)
...
**flacoAi Agent fallback** (only if neither `codex` nor `gemini` is installed)
Launch a second flacoAi Agent (subagent_type: `code-reviewer`, model: `opus`). Log a warning that both reviewers share the same model family — true model diversity was not achieved but context isolation is still enforced.
...
- Reviewer A (flacoAi) always runs — guarantees at least one strong reviewer regardless of tooling.
- Model diversity is the goal for Reviewer B. GPT-5.4 or Gemini 2.5 Pro gives true independence — different training data, different biases, different blind spots. The flacoAi-only fallback still provides value via context isolation but loses model diversity.
