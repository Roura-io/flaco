...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='flacoAi — local AI coding agent powered by Ollama')
    ...
    # Ollama-powered commands
    ollama_chat_parser = subparsers.add_parser('ollama-chat', help='chat with the local Ollama model (no tools)')
    ollama_chat_parser.add_argument('prompt')
    ollama_chat_parser.add_argument('--model', default=None)
    ollama_chat_parser.add_argument('--host', default=None)

    ollama_agent_parser = subparsers.add_parser('ollama-agent', help='agent chat with local Ollama model (with tools)')
    ollama_agent_parser.add_argument('prompt')
    ollama_agent_parser.add_argument('--model', default=None)
    ollama_agent_parser.add_argument('--host', default=None)
    ollama_agent_parser.add_argument('--cwd', default='.')

    subparsers.add_parser('ollama-models', help='list available models on the Ollama server')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'ollama-chat':
        from .ollama_query_engine import OllamaQueryEngine
        engine = OllamaQueryEngine.create(host=args.host, model=args.model)
        for token in engine.chat(args.prompt):
            if token.is_thinking:
                print(f'\033[2m{token.text}\033[0m', end='', flush=True)
            else:
                print(token.text, end='', flush=True)
        print()
        usage = engine.usage
        print(f'\n[{usage.prompt_tokens} in / {usage.completion_tokens} out | {usage.tokens_per_sec:.1f} tok/s]')
        return 0
    if args.command == 'ollama-agent':
        from .ollama_query_engine import OllamaQueryEngine
        engine = OllamaQueryEngine.create(host=args.host, model=args.model, cwd=args.cwd)
        for token in engine.agent_chat(args.prompt):
            if token.is_thinking:
                print(f'\033[2m{token.text}\033[0m', end='', flush=True)
            elif token.is_tool_call:
                if token.tool_result:
                    result_preview = token.tool_result[:200]
                    print(f'\n\033[36m  ← {token.tool_name}: {result_preview}\033[0m', flush=True)
                else:
                    print(f'\n\033[33m  → {token.tool_name}({json.dumps(token.tool_args, default=str)[:100]})\033[0m', flush=True)
            else:
                print(token.text, end='', flush=True)
        print()
        usage = engine.usage
