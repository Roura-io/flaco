...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting flacoAi OAuth login...");
    ...
    println!("flacoAi OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("flacoAi OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "flacoAi OAuth login failed. You can close this window."
    } else {
        "flacoAi OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
impl LiveCli {
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("flaco-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("flaco-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("flaco-issue-body.md", &body)?;
            let output = Command::new("gh")
...
impl DefaultRuntimeClient {
    fn new(
        ...
        let client = resolve_provider_client(&model)?;
        Ok(Self {
...
fn resolve_provider_client(model: &str) -> Result<api::ProviderClient, Box<dyn std::error::Error>> {
    ...
    match provider_kind {
        api::ProviderKind::Ollama => {
            // Ollama doesn't need auth — use ProviderClient which handles this
            Ok(api::ProviderClient::from_model(model)?)
        }
        _ => {
            // For Anthropic/xAI/OpenAI, try auth-based creation first
            let auth = resolve_cli_auth_source().ok();
            match auth {
                Some(auth_source) => Ok(api::ProviderClient::from_model_with_default_auth(
                    model,
                    Some(auth_source),
                )?),
                None => Ok(api::ProviderClient::from_model(model)?),
            }
        }
    }
}
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "flacoAi CLI v{VERSION}")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  flaco                                 Start the interactive REPL"
    )?;
    writeln!(
        out,
        "  flaco \"summarize this repo\"            Run one prompt and exit"
    )?;
    writeln!(
        out,
        "  flaco prompt \"explain src/main.rs\"     Explicit one-shot prompt"
    )?;
    writeln!(
        out,
        "  flaco --resume SESSION.json /status    Inspect a saved session"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  flaco dump-manifests                   Read upstream TS sources and print extracted counts"
    )?;
    writeln!(
        out,
        "  flaco bootstrap-plan                   Print the bootstrap phase skeleton"
    )?;
    writeln!(
        out,
        "  flaco agents                           List configured agents"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  flaco logout                           Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  flaco --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  flaco --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  flaco --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  flaco --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  flaco agents")?;
    writeln!(out, "  flaco /skills")?;
    writeln!(out, "  flaco login")?;
    writeln!(out, "  flaco init")?;
    Ok(())
}
...
mod tests {
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("flaco init"));
        assert!(help.contains("flaco agents"));
        assert!(help.contains("flaco skills"));
        assert!(help.contains("flaco /skills"));
    }
