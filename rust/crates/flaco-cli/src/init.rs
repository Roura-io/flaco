...
const STARTER_FLACOAI_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# flacoAi local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".flacoai/settings.local.json", ".flacoai/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let flacoai_dir = cwd.join(".flacoai");
    artifacts.push(InitArtifact {
        name: ".flacoai/",
        status: ensure_dir(&flacoai_dir)?,
    });

    let flacoai_json = cwd.join(".flacoai.json");
    artifacts.push(InitArtifact {
        name: ".flacoai.json",
        status: write_file_if_missing(&flacoai_json, STARTER_FLACOAI_JSON)?,
    });
    ...
    let flacoai_md = cwd.join("FLACOAI.md");
    let content = render_init_flacoai_md(cwd);
    artifacts.push(InitArtifact {
        name: "FLACOAI.md",
        status: write_file_if_missing(&flacoai_md, &content)?,
    });
...
pub(crate) fn render_init_flacoai_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# FLACOAI.md".to_string(),
        String::new(),
        "This file provides guidance to flacoAi when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.flacoai.json`; reserve `.flacoai/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `FLACOAI.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_flacoai_md};
    use std::fs;
    use std::path::Path;
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".flacoai/        created"));
        assert!(rendered.contains(".flacoai.json    created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("FLACOAI.md       created"));
        assert!(root.join(".flacoai").is_dir());
        assert!(root.join(".flacoai.json").is_file());
        assert!(root.join("FLACOAI.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".flacoai.json")).expect("read flacoai json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".flacoai/settings.local.json"));
        assert!(gitignore.contains(".flacoai/sessions/"));
        let flacoai_md = fs::read_to_string(root.join("FLACOAI.md")).expect("read flacoai md");
        assert!(flacoai_md.contains("Languages: Rust."));
        assert!(flacoai_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("FLACOAI.md"), "custom guidance\n").expect("write existing flacoai md");
        fs::write(root.join(".gitignore"), ".flacoai/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("FLACOAI.md       skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".flacoai/        skipped (already exists)"));
        assert!(second_rendered.contains(".flacoai.json    skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("FLACOAI.md       skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("FLACOAI.md")).expect("read existing flacoai md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".flacoai/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".flacoai/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_flacoai_md(Path::new(&root));
