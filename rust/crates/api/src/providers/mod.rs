...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // Ollama models typically use name:tag format (e.g. qwen3:30b-a3b, llama3:8b)
    // Also detect common Ollama model families not in the registry
    if lower.contains(':')
...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    // Check for Ollama environment variable — if OLLAMA_BASE_URL is set, prefer Ollama
    if std::env::var("OLLAMA_BASE_URL").ok().filter(|v| !v.is_empty()).is_some() {
...
pub fn max_tokens_for_model(model: &str) -> u32 {
    ...
    if let Some(metadata) = metadata_for_model(&canonical) {
        if metadata.provider == ProviderKind::Ollama {
            // Ollama models manage their own context; use a generous output limit
            return 32_768;
        }
    }
