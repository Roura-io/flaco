...
the UNAS shared drive under the user's own folder. Directory scheme:
  ...
  <mount-path>/<user-folder>/flaco/<category>/<filename>
...
- ideas      -- project ideas fleshed out by flaco
- other      -- fallback

### Idea storage

When the user says "save this idea", "i have an idea", "new project idea",
"note this down", or similar, they want flacoAi to **flesh out the idea**
before saving. Do NOT save the raw one-liner -- expand it into a structured
project document.

Before saving, write the content as a markdown document with these sections:
  ...
  ## Key Features
  (bulleted list of 4-8 core features or capabilities)

  ## Technical Approach
  (how you would build this -- stack, architecture, key decisions)

  ## MVP Scope
  (the smallest useful version -- what ships in week 1)

  ## Open Questions
  (2-4 things to figure out before building)

Then save with category "ideas" and filename "<slugified-idea-name>.md".

### General save procedure

If you have the save_to_unas typed tool, use it. Otherwise use bash:
  1. Verify the mount exists:
       `test -d $FLACO_UNAS_MOUNT && echo ok || echo missing`
     If missing, tell the user the UNAS is not mounted on mac-server
  ...
  2. Pick the destination folder:
       DEST=$FLACO_UNAS_MOUNT/<user-folder>/flaco/<category>
  3. Create it:
       `mkdir -p "$DEST"`
  ...
  5. Return a Finder-friendly path AND a Files.app SMB URL, like:
       "Saved to <mount>/<user>/flaco/<category>/<name>.md
        -- SMB URL: smb://10.0.1.2/roura.io.private/<user>/flaco/<category>/<name>.md"
...
under the impression it is on the UNAS is silent data loss.
"#,
    ground_truth_files: &[],
};
