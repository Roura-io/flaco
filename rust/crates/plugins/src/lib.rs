...
const MANIFEST_RELATIVE_PATH: &str = ".flacoai-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("FLACOAI_PLUGIN_ID", &self.plugin_id)
            .env("FLACOAI_PLUGIN_NAME", &self.plugin_name)
            .env("FLACOAI_TOOL_NAME", &self.definition.name)
            .env("FLACOAI_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("FLACOAI_PLUGIN_ROOT", root.display().to_string());
        }
...
mod tests {
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(
            &script_path,
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$FLACOAI_PLUGIN_ID\" \"$FLACOAI_TOOL_NAME\" \"$INPUT\"\n",
        );
        #[cfg(unix)]
        {
